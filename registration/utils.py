...
def validate_password(password, length):
    ...
    if len(password) < length:
        raise forms.ValidationError(_('The password provided must be at least {0} characters.'.format(length)))
    ...
    if cnt < 3:
        raise forms.ValidationError(_('This password does not meet the minimum requirements.'))
